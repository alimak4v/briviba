...
class HistoryManager::Impl {
 public:
  ...
  bool RecordVisit(const std::string& url) {
    if (database_ == nullptr || url.empty() || url == last_recorded_url_) {
      return false;
    }
...
HistoryManager::HistoryManager(std::filesystem::path database_path)
    : impl_(std::make_unique<Impl>(std::move(database_path))) {}

HistoryManager::~HistoryManager() = default;

std::filesystem::path HistoryManager::DefaultDatabasePath() {
  return HomeDirectory() / "Library" / "Application Support" / "Briviba" / "history.sqlite3";
}

bool HistoryManager::RecordVisit(const std::string& url) {
  return impl_->RecordVisit(url);
}
