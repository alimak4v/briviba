...
NSString* VideoTranslateButtonScriptSource() {
  return
      @"(() => {"
       ...
       "})();"
       "const likelyVideoPage = (() => {"
       "  const path = location.pathname.toLowerCase();"
       "  return /(^|\\/)(watch|video|videos|clip|clips|shorts)(\\/|$)/.test(path) ||"
       "         path.includes('/video') || path.includes('/watch') ||"
       "         path.includes('/clip') || path.includes('/clips');"
       "})();"
       "const hasVideo = () => {"
       "  try {"
       ...
       "    button.style.right = fallbackRight + 'px';"
       "    button.style.left = 'auto';"
       "    return;"
       "  }"
       "  const top = Math.max(margin, rect.top + margin);"
       "  const left = Math.max(margin, rect.right - width - margin);"
       "  button.style.top = top + 'px';"
       "  button.style.left = left + 'px';"
       "  button.style.right = 'auto';"
       "};"
       "const ensureStyles = (button) => {"
       ...
       "};"
       "const ensureButton = () => {"
       "  if (!supportedHost || (!likelyVideoPage && !hasVideo())) {"
       "    removeButton();"
       "    return;"
       "  }"
       ...
       "  button.id = buttonId;"
       "  button.type = 'button';"
       "  button.title = 'Перевести видео через Яндекс';"
       "  button.setAttribute('aria-label', 'Перевести видео через Яндекс');"
       "  ensureStyles(button);"
       ...
       "    'height: 18px',"
       "    'border-radius: 999px',"
       "    'background: linear-gradient(135deg, #ffcc33, #ff6b3d)',"
       "    'color: white',"
       ...
       "      if (button) positionButton(button);"
       "    });"
       "  } catch (_) {"
       "    ensureButton();"
       "    const button = document.getElementById(buttonId);"
